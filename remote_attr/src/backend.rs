...
pub fn impl_backend_remote(ast: ItemFn) -> TokenStream {
    ...
    let statements: Vec<Stmt> = parse_quote! {
        let #input_args_as_tuple: #input_args_as_type_tuple = rust_wasm_rpcgen::bincode::deserialize(bytes).unwrap();

        #inner_fn_stmt

        let call_output = #inner_fn_ident(#inner_fn_args);
        let encoded: Vec<u8> = rust_wasm_rpcgen::bincode::serialize(&call_output).unwrap();
